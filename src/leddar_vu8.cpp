...
#define LEDDAR_VU8_LOG(level, args) \
    { \
        std::stringstream ss; \
        ss << args; \
...
#define LEDDAR_VU8_LOG_ERROR(args) \
    LEDDAR_VU8_LOG(leddar_vu8::kLogLevelError, args)

#define LEDDAR_VU8_LOG_INFO(args) \
    LEDDAR_VU8_LOG(leddar_vu8::kLogLevelInfo, args)

class LockGuard {
public:
    LockGuard(std::mutex &mutex) : mutex(mutex) {
        mutex.lock();
    }
...
static bool send_can_data(int sock, uint32_t message_id, const uint8_t (&data)[8]) {
    ...
    if (rc != sizeof(frame)) {
        LEDDAR_VU8_LOG_ERROR(
            "send can frame failed w/ errno " << errno << " - " << strerror(errno)
        );
        return false;
    }
    return true;
...
static bool recv_can_data(int sock, uint32_t &message_id, uint8_t (&data)[8]) {
    ...
    if (rc != sizeof(frame)) {
        LEDDAR_VU8_LOG_ERROR(
            "recv can frame failed w/ errno " << errno << " - " << strerror(errno)
        );
        return false;
    }
    ...
    if (frame.can_dlc != 8) {
        LEDDAR_VU8_LOG_ERROR(
            "can frame data size " << frame.can_dlc << " != " << 8
        );
        return false;
    }
...
static bool recv_answer(int sock, uint32_t message_id, uint8_t (&data)[8]) {
    static const uint8_t error[6] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};

    uint32_t frame_message_id;
    if (!recv_can_data(sock, frame_message_id, data)) {
        return false;
    }
    if (frame_message_id  != message_id) {
        LEDDAR_VU8_LOG_ERROR(
            "can frame message id " << frame_message_id << " != " << message_id
        );
        return false;
    }
    if (memcmp(data + 2, error, 6) == 0) {
        LEDDAR_VU8_LOG_ERROR("can request failed");
        return false;
    }
    return true;
}
...
void  Echo::from_data(const uint8_t (&data)[8]) {
    ...
    amplitude = ((double) TO_UINT16(data[2], data[3])) / 64.0;
...
bool Stream::Start() {
    ...
    if ( sock < 0 ) {
        LEDDAR_VU8_LOG_ERROR(
            "failed to create socket w/ errno " << errno << " - " << strerror(errno)
        );
        goto cleanup;
    }
    ...
    if (timeout_ != 0) {
        struct timeval tv = {
            int(timeout_),
            int((timeout_ - int(timeout_)) * 1e6) // micro-seconds
        };
        rc = setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));
        if (rc < 0) {
            LEDDAR_VU8_LOG_ERROR(
                "set socket recv timeout " << timeout_ << " failed w/ errno " << errno << " - " << strerror(errno)
            );
            goto cleanup;
        }
        LEDDAR_VU8_LOG_INFO(
            "set socket recv timeout to " << timeout_
        );
    }
    ...
    if (interface_.size() + 1 > IF_NAMESIZE) {
        LEDDAR_VU8_LOG_ERROR(
            "interface " << interface_ << " length > " << IF_NAMESIZE
        );
    }
    ...
    if (rc == -1) {
        LEDDAR_VU8_LOG_ERROR(
            "find interface " << interface_ << " index failed w/ errno " << errno << " - " << strerror(errno)
        );
        goto cleanup;
    }
    ...
    if (rc == -1) {
        LEDDAR_VU8_LOG_ERROR(
            "bind socket to interface " << interface_ << " failed w/ errno " << errno << " - " << strerror(errno)
        );
        goto cleanup;
    }
    ...
    if (notify == -1) {
        LEDDAR_VU8_LOG_ERROR(
            "notify eventfd failed w/ errno " << errno << " - " << strerror(errno)
        );
        goto cleanup;
    }
...
void Stream::Stop() {
    if (is_listening()) {
        LEDDAR_VU8_LOG_INFO("notify stream thread");
        eventfd_write(notify_, 1);
    }

    if (thd_.joinable()) {
        LEDDAR_VU8_LOG_INFO("join stream thread " << thd_.get_id());
        thd_.join();
    }

    if (sock_ != -1) {
        LEDDAR_VU8_LOG_INFO("closing stream socket 0x" << std::hex << sock_);
        close(sock_);
        sock_ = -1;
    }

    if (notify_ != -1) {
        LEDDAR_VU8_LOG_INFO("closing notify event 0x" << std::hex << notify_);
        close(notify_);
        notify_ = -1;
    }
}
...
void Stream::Listen() {
    ...
    while (true) {
        ...
        if (rc == -1) {
            LEDDAR_VU8_LOG_ERROR(
                "failed select w/ errno " << errno << " - " << strerror(errno)
            );
            break;
        }
        ...
        if (FD_ISSET(notify_, &rfds)) {
            ...
            if (rc == -1) {
                LEDDAR_VU8_LOG_ERROR(
                    "failed to read notify event w/ errno " << errno << " - " << strerror(errno)
                );
                break;
            }
            break;
...
bool Sensor::Connect() {
    ...
    if (send_timeout_ == 0) {
        struct timeval lTimeVal = {
            int(send_timeout_),
            int((send_timeout_ - int(send_timeout_)) * 1e6) // micro-seconds
        };
        rc = setsockopt(sock, SOL_SOCKET, SO_SNDTIMEO, &lTimeVal, sizeof( lTimeVal ));
        if (rc < 0) {
            LEDDAR_VU8_LOG_ERROR(
                "failed to set socket send timeout w/ errno " << errno << " - " << strerror(errno)
            );
            goto cleanup;
        }
        LEDDAR_VU8_LOG_INFO(
            "set socket send timeout to " << send_timeout_
        );
    }
    ...
    if (recv_timeout_ != 0) {
        struct timeval tv = {
            int(recv_timeout_),
            int((recv_timeout_ - int(recv_timeout_)) * 1e6) // micro-seconds
        };
        rc = setsockopt( sock, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv) );
        if (rc < 0) {
            LEDDAR_VU8_LOG_ERROR(
                "set socket recv timeout " << recv_timeout_ << " failed w/ errno " << errno << " - " << strerror(errno)
            );
            goto cleanup;
        }
        LEDDAR_VU8_LOG_INFO(
            "set socket recv timeout to " << recv_timeout_
        );
    }
...
bool Sensor::StartStream() {
    uint8_t data[8];

    memset(data, 0, sizeof(data));
    data[0] = 3;
    data[1] = 1;  // FIXME: multi/single message mode
    if (!send_can_data(sock_, config_.base_rx_message_id(), data)) {
        return false;
    }

    memset(data, 0, sizeof(data));
    if (!recv_answer(sock_, config_.base_tx_message_id(), data)) {
        return false;
    }
...
bool Config::Load() {
    ...
    data[0] = 5;
    data[1] = 0;
    if (!send_can_data(sensor_.sock(), base_rx_message_id_, data)) {
        return false;
    }
    if (!recv_answer(sensor_.sock(), base_tx_message_id_, data)) {
        return false;
    }
    accumulation_exponent_ = data[2];
    LEDDAR_VU8_LOG_INFO(
        "loaded accumulation_exponent=" << unsigned(accumulation_exponent_)
    );
    oversampling_exponent_ = data[3];
    LEDDAR_VU8_LOG_INFO(
        "loaded oversampling_exponent=" << unsigned(oversampling_exponent_)
    );
    base_samples_ = data[4];
    LEDDAR_VU8_LOG_INFO("loaded base_samples=" << unsigned(base_samples_));
    ...
    smoothing_ = data[2];
    LEDDAR_VU8_LOG_INFO("loaded smoothing=" << signed(smoothing_));
    detection_threshold_ = TO_UINT32(data[4], data[5], data[6], data[7]);
    LEDDAR_VU8_LOG_INFO("loaded detection_threshold=" << detection_threshold_);
    ...
    light_source_power_percent_ = data[2];
    LEDDAR_VU8_LOG_INFO(
        "loaded light_source_power_percent_=" << light_source_power_percent_
    );
    saturation_count_ = data[3];
    LEDDAR_VU8_LOG_INFO(
        "loaded saturation_count=" << unsigned(saturation_count_)
    );
    auto_light_source_power_ = TO_UINT16(data[4], data[5]);
    LEDDAR_VU8_LOG_INFO(
        "loaded auto_light_source_power=" << auto_light_source_power_
    );
    ...
    distance_resolution_ = TO_UINT16(data[2], data[3]);
    distance_resolution_ = 100;
    LEDDAR_VU8_LOG_INFO("loaded distance_resolution=" << distance_resolution_);
    acquisition_options_ = TO_UINT16(data[4], data[5]);
    LEDDAR_VU8_LOG_INFO(
        "loaded auto_light_source_power_enabled=" <<
        auto_light_source_power_enabled()
    );
    LEDDAR_VU8_LOG_INFO(
        "loaded demerge_object_enabled=" << demerge_object_enabled()
    );
    LEDDAR_VU8_LOG_INFO(
        "loaded static_noise_removal_enabled=" <<
        static_noise_removal_enabled()
    );
    LEDDAR_VU8_LOG_INFO("loaded precision_enabled=" << precision_enabled());
    LEDDAR_VU8_LOG_INFO(
        "loaded saturation_compenstation_enabled=" <<
        saturation_compenstation_enabled());
    LEDDAR_VU8_LOG_INFO(
        "loaded overshoot_management_enabled=" <<
        overshoot_management_enabled()
    );
    ...
    baud_rate_ = (BaudRate) data[2];
    LEDDAR_VU8_LOG_INFO("loaded baud_rate=" << baud_rate_);
    frame_format_ = (FrameFormat) data[3];
    LEDDAR_VU8_LOG_INFO("loaded frame_format=" << frame_format_);
    base_tx_message_id_ = TO_UINT32(data[4], data[5], data[6], data[7]);
    LEDDAR_VU8_LOG_INFO(
        "loaded base_tx_message_id=0x" << std::hex << base_tx_message_id_
    );
    memset(data, 0, 8);
    data[0] = 5;
    data[1] = 5;
    ...
    base_rx_message_id_ = TO_UINT32(data[4], data[5], data[6], data[7]);
    LEDDAR_VU8_LOG_INFO(
        "loaded base_rx_message_id=0x" << std::hex << base_rx_message_id_
    );
    memset(data, 0, 8);
    data[0] = 5;
    data[1] = 6;
    ...
    max_detections_ = data[3];
    LEDDAR_VU8_LOG_INFO("loaded max_detections=" << unsigned(max_detections_));
    inter_message_delay_ = TO_UINT16(data[4], data[5]);
    LEDDAR_VU8_LOG_INFO("loaded inter_message_delay=" << inter_message_delay_);
    inter_cycle_delay_ = TO_UINT16(data[6], data[7]);
    LEDDAR_VU8_LOG_INFO("loaded inter_cycle_delay=" << inter_cycle_delay_);
    ...
    enabled_segments_ = TO_UINT32(data[4], data[5], data[6], data[7]);
    LEDDAR_VU8_LOG_INFO("loaded enabled_segments=" << enabled_segments_);
...
bool Config::Save() {
    ...
    data[0] = 6;
    data[1] = 0;
    data[2] = accumulation_exponent_;
    LEDDAR_VU8_LOG_INFO(
        "saving accumulation_exponent=" << unsigned(accumulation_exponent_)
    );
    data[3] = oversampling_exponent_;
    LEDDAR_VU8_LOG_INFO(
        "saving oversampling_exponent=" << unsigned(oversampling_exponent_)
    );
    data[4] = base_samples_;
    LEDDAR_VU8_LOG_INFO("saving base_samples=" << unsigned(base_samples_));
    if (!send_can_data(sensor_.sock(), base_rx_message_id_, data)) {
        return false;
    }
    if (!recv_answer(sensor_.sock(), base_tx_message_id_, data)) {
    ...
    data[1] = 1;
    data[2] = smoothing_;
    LEDDAR_VU8_LOG_INFO("saving smoothing=" << signed(smoothing_));
    FROM_UINT32(detection_threshold_, data[4], data[5], data[6], data[7]);
    LEDDAR_VU8_LOG_INFO("saving detection_threshold=" << detection_threshold_);
    if (!send_can_data(sensor_.sock(), base_rx_message_id_, data)) {
        return false;
    }
    if (!recv_answer(sensor_.sock(), base_tx_message_id_, data)) {
    ...
    data[0] = 6;
    data[1] = 2;
    data[2] = light_source_power_percent_;
    LEDDAR_VU8_LOG_INFO(
        "saving light_source_power_percent=" << light_source_power_percent_
    );
    data[3] = saturation_count_;
    LEDDAR_VU8_LOG_INFO(
        "saving saturation_count=" << unsigned(saturation_count_)
    );
    FROM_UINT16(auto_light_source_power_, data[4], data[5]);
    if (!send_can_data(sensor_.sock(), base_rx_message_id_, data)) {
        return false;
    ...
    data[0] = 6;
    data[1] = 3;
    FROM_UINT16(distance_resolution_, data[2], data[3]);
    LEDDAR_VU8_LOG_INFO("saving distance_resolution=" << distance_resolution_);
    FROM_UINT16(acquisition_options_, data[4], data[5]);
    LEDDAR_VU8_LOG_INFO(
        "saving auto_light_source_power_enabled=" <<
        auto_light_source_power_enabled()
    );
    LEDDAR_VU8_LOG_INFO(
        "saving demerge_object_enabled=" << demerge_object_enabled()
    );
    LEDDAR_VU8_LOG_INFO(
        "saving static_noise_removal_enabled=" <<
        static_noise_removal_enabled()
    );
    LEDDAR_VU8_LOG_INFO("saving precision_enabled=" << precision_enabled());
    LEDDAR_VU8_LOG_INFO(
        "saving saturation_compenstation_enabled=" <<
        saturation_compenstation_enabled());
    LEDDAR_VU8_LOG_INFO(
        "saving overshoot_management_enabled=" <<
        overshoot_management_enabled()
    );
    if (!send_can_data(sensor_.sock(), base_rx_message_id_, data)) {
        return false;
    ...
    data[0] = 6;
    data[1] = 6;
    data[3] = max_detections_;
    LEDDAR_VU8_LOG_INFO("saving max_detections=" << unsigned(max_detections_));
    FROM_UINT16(inter_message_delay_, data[4], data[5]);
    LEDDAR_VU8_LOG_INFO("saving inter_message_delay=" << inter_message_delay_);
    FROM_UINT16(inter_cycle_delay_, data[6], data[7]);
    LEDDAR_VU8_LOG_INFO("saving inter_cycle_delay=" << inter_cycle_delay_);
    if (!send_can_data(sensor_.sock(), base_rx_message_id_, data)) {
        return false;
    }
    if (!recv_answer(sensor_.sock(), base_tx_message_id_, data)) {
    ...
    data[0] = 6;
    data[1] = 8;
    FROM_UINT32(enabled_segments_, data[4], data[5], data[6], data[7]);
    LEDDAR_VU8_LOG_INFO("saving enabled_segments=" << enabled_segments_);
    if (!send_can_data(sensor_.sock(), base_rx_message_id_, data)) {
        return false;
    }
    if (!recv_answer(sensor_.sock(), base_tx_message_id_, data)) {
